...
#endif
{
	...
	g_pBuf=(LPTSTR)HeapAlloc(hProcHeap, HEAP_ZERO_MEMORY, sizeof(TCHAR)*_MAX_NTFS_PATH);
	if (g_pBuf==NULL)
	{
	...
	ofn.lpstrFilter = _T("All\0*.*\0PFX files\0*.PFX\0");
	ofn.nFilterIndex = 2;
	ofn.Flags = OFN_PATHMUSTEXIST | OFN_FILEMUSTEXIST | OFN_HIDEREADONLY;
	if (!GetOpenFileName(&ofn)) goto CLEANUP;

	hFile = CreateFile(g_pBuf, GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
...
ASKPASSWORD:
	INT_PTR ret=DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_PASSWDIALOG), HWND_DESKTOP, (DLGPROC)PasswordDialogProc);
	if (ret!=IDOK) goto CLEANUP;
...
CLEANUP:
	...
	if (g_pBuf) HeapFree(hProcHeap, 0, g_pBuf);
